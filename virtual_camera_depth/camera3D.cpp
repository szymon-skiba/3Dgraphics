...
bool compareFaces(const std::pair<const Cube*, int>& a, const std::pair<const Cube*, int>& b) {
    const Vector4 centerA = a.first->faceCenter(a.second);
    const Vector4 centerB = b.first->faceCenter(b.second);
...
void drawCube(const Cube& cube) {
    std::vector<std::pair<const Cube*, int>> sortedFaces;

    for (int i = 0; i < 6; ++i) {
        sortedFaces.emplace_back(&cube, i);
    }

    std::sort(sortedFaces.begin(), sortedFaces.end(), compareFaces);

    for (const auto& face : sortedFaces) {
        const auto& indices = cube.faces[face.second];
        glBegin(GL_POLYGON);
        switch(face.second) {
        ...
        for (int index : indices) {
            const auto& vertex = cube.vertices[index];
